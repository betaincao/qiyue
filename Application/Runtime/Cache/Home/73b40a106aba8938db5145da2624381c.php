  ...
  <ul id="collect">
      <?php if(is_array($OrderForm)): foreach($OrderForm as $key=>$vo): ?><li>
         <h5>编号：<?php echo ($vo["a_num"]); ?></h5>
          <hr/>
        <button class="button" onclick="show()"  ></button>
       <a href="/qiyue/index.php/Home/ApplianceInfo/ApplianceInfo/a_id/<?php echo ($vo["a_id"]); ?>/a_name/<?php echo ($vo["a_name"]); ?>/a_photo/'/qiyue/Public/Images/book.jpg'/l_name/计算机应用技术协会/a_num/XX55-3"><img src="/qiyue/Public/Images/cloth.jpg" /></a>
        ...
        <p class="message-end"><?php echo (date('Y-m-d H:i:s',$vo["o_stoptime"])); ?></p>
         <p class="message-start"><?php echo (date('Y-m-d H:i:s',$vo["o_starttime"])); ?></p>
...
<!-- /qiyue/index.php/Home/Comment/WriteComment/a_id/<?php echo ($vo['a_id']); ?> -->
  ...
  <div id="shade">
     ...
     <form name="comment" data-aId="<?php echo ($vo['a_id']); ?>" data-oId="<?php echo ($vo['o_id']); ?>"  method='POST'>
         <textarea cols="4" rows="8" id="comment"></textarea>
         <input type="button" value="下次再说" style="background-color: #FFCCCC;" onclick="display();" />
   ...
   function loadXMLDoc()
    {
      ...
      var texta = document.getElementsByTagName('form')[0].getAttribute("data-aId");
      var texto = document.getElementsByTagName('form')[0].getAttribute("data-oId");
      console.log(texta);
      console.log(texto);
