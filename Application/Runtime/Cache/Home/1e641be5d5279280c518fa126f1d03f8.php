    ...
    <!-- 这里实现的是我的评论 -->
     <ul id="collect" class="choice-box">
      <?php if(is_array($Comment)): foreach($Comment as $key=>$vo): ?><li>
          <h5><?php echo ($vo["a_name"]); ?></h5>
          <hr/>
          <!-- <img src="<?php echo ($vo["a_photo"]); ?>" /> -->
          <span class="state" style="display: none;"><?php echo ($vo["c_state"]); ?></span>
          <img src="/qiyue/Public/Images/cloth.jpg" />
        <div>
          <span class=""></span>
          <P class="content"><?php echo ($vo["c_comment"]); ?></P>
        </div>
        <p class="time"><?php echo (date('Y-m-d H:i:s',$vo["c_time"])); ?></p>
        <p class="mode"></p>
