...
<div class="content">
    <!-- 主图 -->
    <img src="/qiyue/Public/Images/model1.png">

    <div id="main">
        <div class="item"><span>名称:</span><?php echo ($vo["a_name"]); ?></div>
        <div class="item"><span>简介:</span><?php echo ($vo["a_detail"]); ?></div>
        <div class="item"><span>仪器编号:</span><?php echo ($vo["a_num"]); ?></div>
        <div class="item"><span>所属实验室:</span><?php echo ($vo["l_name"]); ?></div>
        <div class="item"><span>预约次数:</span><?php echo ($vo["a_add"]); ?>次</div>
    <div class="bottom">
    ...
    <script type="text/javascript"> 
       $(function(){
           $('#ajax').click(function(){                 //绑定失去焦点方法
               ...
               if(value1 == "2") {window.location.href='/qiyue/index.php/Home/Login/login'}
               if(value1!=2){
                   if ($('#ajax').html()== "收藏") {
                        $('#ajax').html("已收藏");  //1
                        var state = 1;
               ...
               $.get('/qiyue/index.php/Home/Conllect/UpdateConllect/',{'state':state,'a_id':<?php echo ($vo["a_id"]); ?>},function(){  
               });
           });
     });
        ...
        <div class="item">
            <span style="margin-bottom: 10px;">评论</span><br>
            <?php if(is_array($Comment)): foreach($Comment as $key=>$vo): ?><div class="coment">
                    <h4><?php echo ($vo["u_name"]); ?></h4>
                    <span class="time"><?php echo (date('Y-m-d H:i:s',$vo["c_time"])); ?></span>
                    <p><?php echo ($vo["c_comment"]); ?></p>
